...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Título del inmueble' }).fill('local en ibague la esperanza');
  await page.getByRole('spinbutton', { name: 'Precio' }).click();
  await page.getByRole('spinbutton', { name: 'Precio' }).fill('1500000');
  ...
  await page.getByRole('textbox', { name: 'Ciudad o municipio' }).fill('valle lindo');
  await page.getByRole('textbox', { name: 'Provincia' }).click();
  await page.getByRole('textbox', { name: 'Provincia' }).fill('Manizales');
