...
test('test', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Inmuebles' }).click();
  const inmuebleCreado = page.getByText(datos.inmueble.tituloBorrar);
  await expect(inmuebleCreado).toBeVisible();
  await inmuebleCreado.click();
  await page.getByRole('button', { name: 'Eliminar inmueble' }).click();
  await page.getByRole('button', { name: 'Aceptar' }).click();
  await expect(page.getByText('Inmueble eliminado correctamente')).toBeVisible();
  await page.getByRole('link', { name: 'Inmuebles' }).click();
  const inmuebleEliminado = page.getByText(datos.inmueble.tituloBorrar);
  await expect(inmuebleCreado).not.toBeVisible();
  await page.close();
})