...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Título del inmueble' }).fill('Chalet en venta en Ontinyent');
  await page.getByRole('spinbutton', { name: 'Precio' }).click();
  await page.getByRole('spinbutton', { name: 'Precio' }).fill('42000');
  ...
  await page.getByRole('textbox', { name: 'Ciudad o municipio' }).fill('Olleria');
  await page.getByRole('textbox', { name: 'Provincia' }).click();
  await page.getByRole('textbox', { name: 'Provincia' }).fill('Valencia');
